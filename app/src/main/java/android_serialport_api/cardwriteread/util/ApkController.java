...
public class ApkController {
    ...
    public static boolean install(String apkPath, Context context){
        ...
        if(hasRootPerssion()){
            // 有root权限，利用静默安装实现
            System.out.println("有root权限，利用静默安装实现");
            return clientInstall(apkPath);
        }else{
            // 没有root权限，利用意图进行安装
            System.out.println(" 没有root权限，利用意图进行安装");
            ...
            Uri uri =  FileProvider.getUriForFile(context, "android_serialport_api.cardwriteread.fileprovider", file);
